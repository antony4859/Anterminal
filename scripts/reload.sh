...
print_tag_cleanup_reminder() {
  ...
  if [[ "${#stale_tags[@]}" -eq 0 ]]; then
    ...
    for tag in "${stale_tags[@]}"; do
      echo "  pkill -f \"anterminal DEV ${tag}.app/Contents/MacOS/anterminal DEV\""
  ...
  echo "  pkill -f \"anterminal DEV ${current_slug}.app/Contents/MacOS/anterminal DEV\""
