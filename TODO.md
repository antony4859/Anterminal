...
## Embedded Web Server (remote access via Tailscale)
- [ ] **P0** Add PTY session persistence and WebSocket reconnect — keep PTY alive for 60s after WS disconnect, allow re-attach by session ID, add reconnect logic to client JS (files: `Sources/PTYWebSocket.swift`, `Sources/EmbeddedServer.swift`, `Sources/EmbeddedServerHTML.swift`)
- [ ] **P1** Add PTY session reaper and WebSocket heartbeat — implement ping/pong, add periodic sweep in `PTYSessionManager` to clean up orphaned sessions past grace period (files: `Sources/PTYWebSocket.swift`, `Sources/EmbeddedServer.swift`) [blocked by PTY persistence]
- [ ] **P1** Wire up command acknowledgment for REST and WebSocket endpoints — stop discarding `handleBridgeCommand` callback results, wait for completion before returning HTTP response, send WS command results back to client (file: `Sources/EmbeddedServer.swift`)
- [ ] **P2** Bundle xterm.js locally instead of loading from CDN — download xterm.js + addon-fit + addon-web-links, serve from embedded server so the UI works without general internet (files: `Sources/EmbeddedServerHTML.swift`, `Sources/EmbeddedServer.swift`)
- [ ] **P2** Push tmux state via WebSocket broadcast instead of polling — include tmux sessions in the 2s `/ws` state broadcast, remove the 5s `setInterval` fetch from client JS (files: `Sources/EmbeddedServer.swift`, `Sources/EmbeddedServerHTML.swift`)
