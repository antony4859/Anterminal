...
class TabManager: ObservableObject {
    ...
    @discardableResult
    func addTab(select: Bool = true, isTmux: Bool = false) -> Workspace { addWorkspace(select: select, isTmux: isTmux) }
    ...
    /// Toggle zoom on a panel — fills the entire workspace area with just this panel.
    /// Press Cmd+Shift+Enter to toggle. Like tmux prefix+z.
    func toggleSplitZoom(tabId: UUID, surfaceId: UUID) -> Bool {
        guard let workspace = tabs.first(where: { $0.id == tabId }) else { return false }
        if workspace.zoomedPanelId == surfaceId {
            // Un-zoom
            workspace.zoomedPanelId = nil
        } else {
            // Zoom this panel
            workspace.zoomedPanelId = surfaceId
        }
        return true
    }
