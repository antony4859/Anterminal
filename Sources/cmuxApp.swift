...
struct cmuxApp: App {
    ...
    private static func terminateForMissingLaunchTag() -> Never {
        let message = "error: refusing to launch untagged anterminal DEV; start with ./scripts/reload.sh --tag <name> (or set CMUX_TAG for test harnesses)"
        fputs("\(message)\n", stderr)
        fflush(stderr)
        NSLog("%@", message)
        Darwin.exit(64)
    ...
    var body: some Scene {
        ...
        .commands {
            ...
            CommandGroup(after: .toolbar) {
                ...
                Button("Zoom Panel") {
                    let manager = activeTabManager
                    if let tabId = manager.selectedTabId,
                       let surfaceId = manager.focusedSurfaceId(for: tabId) {
                        _ = manager.toggleSplitZoom(tabId: tabId, surfaceId: surfaceId)
                    }
                }
    ...
    private func editor(for kind: SettingsAboutWindowKind) -> some View {
        ...
        return GroupBox(kind.displayTitle) {
            VStack(alignment: .leading, spacing: 10) {
                ...
                Text("When disabled, anterminal uses normal default titlebar behavior for this window.")
                    .font(.caption)
...
private struct AboutPanelView: View {
    ...
    private let githubURL = URL(string: "https://github.com/manaflow-ai/anterminal")
    private let docsURL = URL(string: "https://anterminal.dev/docs")
            ...
            VStack(alignment: .center, spacing: 32) {
                ...
                VStack(spacing: 2) {
                    ...
                    let commitURL = commit.flatMap { hash in
                        URL(string: "https://github.com/manaflow-ai/anterminal/commit/\(hash)")
                    }
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 14) {
                    ...
                    SettingsCard {
                        ...
                        SettingsCardRow(
                            "Open Sidebar PR Links in anterminal Browser",
                            subtitle: openSidebarPullRequestLinksInCmuxBrowser
                                ? "Clicks open inside anterminal browser."
                    ...
                    SettingsCard {
                        SettingsCardRow(
                            ...
                            subtitle: claudeCodeHooksEnabled
                                ...
                                : "Claude Code runs without anterminal integration."
                        ) {
                        ...
                        SettingsCardNote("When enabled, anterminal wraps the claude command to inject session tracking and notification hooks. Disable if you prefer to manage Claude Code hooks yourself.")
                    }
                    ...
                    SettingsCard {
                        ...
                        SettingsCardRow(
                            "Open Terminal Links in anterminal Browser",
                        ...
                        if openTerminalLinksInCmuxBrowser || interceptTerminalOpenCommandInCmuxBrowser {
                            ...
                            VStack(alignment: .leading, spacing: 6) {
                                SettingsCardRow(
                                    ...
                                    subtitle: "Applies to terminal link clicks and intercepted `open https://...` calls. Only these hosts open in anterminal. Others open in your default browser. One host or wildcard per line (for example: example.com, *.internal.example). Leave empty to open all hosts in anterminal."
                                ) {
                                    EmptyView()
                                }
                        ...
                        VStack(alignment: .leading, spacing: 8) {
                            ...
                            Text("Controls which HTTP (non-HTTPS) hosts can open in anterminal without a warning prompt. Defaults include localhost, 127.0.0.1, ::1, 0.0.0.0, and *.localtest.me.")
                                .font(.caption)
                                .foregroundStyle(.secondary)
